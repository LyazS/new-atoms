...
- 如果任务需要新增第三方依赖或安装包，直接修改 `/package.json` 中的 `dependencies` 或 `devDependencies` 即可；运行环境会自动检测变更并安装这些依赖，不需要假设存在 shell、npm、pnpm 或 yarn 命令
- 当你创建或重构关键页面区块、按钮、卡片、标题、导航、表单等重要节点时，优先补充稳定的 `data-btoms-id` 属性，便于后续在预览区进行点选式编辑；命名应语义化、简短且稳定，例如 `hero-title`、`pricing-card-pro`、`cta-primary`

### 当前工作目录与查询方式
- 你的当前工作目录是虚拟工作区根目录，可视为 `/`
- 所有查询和修改都只能基于这个虚拟工作区进行
- 不要使用真实机器路径，也不要假设可以访问工作区之外的内容
- 默认新会话通常会包含这些关键文件：`/package.json`、`/index.html`、`/vite.config.ts`、`/src/App.vue`、`/src/main.js`、`/src/styles.css`；但在实际操作前，仍应优先通过 `list_files` 或 `read_file` 确认当前工作区的真实状态
...
- 如果用户输入中包含 `selection_context`，表示用户在预览区点选了一个具体元素；此时你应优先围绕该节点或其最近语义父区块做局部修改，除非用户明确要求全局改版，否则不要大范围重写无关区域
- 当存在 `selection_context` 但定位不够稳定时，先通过 `read_file` 查找对应文案、属性、`data-btoms-id`、类名或区块结构，再执行最小必要修改
...
def _build_selection_context_block(selection_context: SelectedNodeContext) -> str:
    payload = {
        "tagName": selection_context.tagName,
        "textPreview": selection_context.textPreview,
        "role": selection_context.role,
        "ariaLabel": selection_context.ariaLabel,
        "id": selection_context.id,
        "classList": selection_context.classList,
        "dataBtomsId": selection_context.dataBtomsId,
        "boundingRect": selection_context.boundingRect.model_dump(mode="json"),
        "parentSummary": selection_context.parentSummary,
        "sectionSummary": selection_context.sectionSummary,
        "suggestedSelector": selection_context.suggestedSelector,
    }
    return json.dumps(payload, ensure_ascii=False, indent=2)


def wrap_user_task(content: str, selection_context: SelectedNodeContext | None = None) -> str:
    ...
    return (
        "<task>\n"
        f"用户要求：{content}\n\n"
        "当前用户在预览区点选了一个元素，请优先在该区域附近做局部修改。\n"
        "除非用户明确要求整体改版，否则不要大范围改动无关区块。\n\n"
        "<selection_context>\n"
        f"{_build_selection_context_block(selection_context)}\n"
        "</selection_context>\n\n"
        "修改约束：优先局部修改，尽量保留周围布局与内容结构；如果缺少稳定定位锚点，先补充或利用 `data-btoms-id`。\n"
        "</task>"
    )
...
class AgentRunner:
    ...
    def _build_openai_messages(self, session: Session) -> list[ChatCompletionMessageParam]:
        ...
        for message in session.messages:
            content = (
                wrap_user_task(message.content, message.selection_context)
